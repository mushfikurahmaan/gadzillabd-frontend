...
export function generateOrderPDF(data: OrderPDFData): void {
  ...
  addCenteredText('GADZILLA', yPos, 24);
  yPos += 8;

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  addCenteredText('Order Submission Receipt', yPos, 12);
  yPos += 15;
  ...
  addCenteredText('Thank you for shopping with GADZILLA!', yPos, 11);
  yPos += 6;
  addCenteredText('For any queries, please contact us.', yPos, 10);
  yPos += 6;
  addCenteredText('gadzillabd@gmail.com', yPos, 10);

  // Save the PDF
  doc.save(`GADZILLA_Order_${data.orderId}.pdf`);
}
